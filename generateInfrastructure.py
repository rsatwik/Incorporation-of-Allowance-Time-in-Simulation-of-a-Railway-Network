...
class Loop():
    def __init__(self,station,UDC,ML=False,count=1,bwt=4):
        #assert UDC in [0,1,2]
        #assert count<=9
    ...
    def __repr__(self):
        ...
        dnlinkText,dnlinkLengthText,dnlinkPriorityText,dnlinkcrossoverText,speedRestrictionText,self.bwt) #"Kanpur" 0.0 1.0 100.0

class Block():
    def __init__(self,prevStation,nextStation,UDC,startDist,endDist,maxAllowedSpeed=160,count=1,ML=0,bwt = 4):
        # ML = True
...
class Network():
    ...
    def fillBlocks(self,blkDetails,DYheadWayDistance =1.2,minLength = 1.2,maxLength = 1.8):
        ...
        for p,n in zip(self.stationList[:-1],self.stationList[1:]):
            ...
            #add block working time
            bwt=4
            if 'UpBWT' in blkDetails.columns:
                upbwt = blkDetails[blkDetails['Station']==blkName].UpBWT.values[0]
                dnbwt = blkDetails[blkDetails['Station']==blkName].DnBWT.values[0]

            blockCount = 1
            #distance Calculation
            dist = self.blockLength(blkName)
            if 'BlockCount' in blkDetails.columns:
                blockCount = blkDetails[blkDetails['Station']==blkName].BlockCount.values[0]

            if True:
                #print('absoluteBlock!!',blkName,dist)
                DYheadWayDistance = dist/blockCount
            else: # Redundant code for automatic signalling
            ...
            for blkLine,lineCount in requiredBlocks:
                ...
                while endDist < n.startDist-1:
                    if blkLine == 'up':
                        ...
                        upBlock = Block(p,n,0,startDist,endDist,maxAllowedSpeed=maxAllowedUpSpeed,count = count,ML=lineCount,bwt=upbwt)
                        ...
                        dnBlock = Block(p,n,1,startDist,endDist,maxAllowedSpeed=maxAllowedDnSpeed,count = count,ML=lineCount,bwt=dnbwt)
                        ...
                        upBlock = Block(p,n,2,startDist,endDist,maxAllowedSpeed=maxAllowedUpSpeed,count = count,ML=lineCount,bwt=upbwt)
                        dnBlock = upBlock
                        self.blockList.append(upBlock)
                        self.graph.edges[p.name,n.name]['upBlocks'].append(upBlock)
